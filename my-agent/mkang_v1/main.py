...
KYUREM, SMOOCHUM, LATIAS = 144, 183, 184
CONKELDURR, ANNIHILAPE, FEZANDIPITI, MEOWTH = 115, 224, 140, 1071
PSY, TELEPATH = 5, 19
NIGHTSTRETCHER, ULTRABALL, POKEPAD, CIPHER = 1097, 1121, 1152, 1188
LILLIE, ACADEMY, PATCH, PRIMECATCHER = 1227, 1248, 1146, 1088
SWITCH, COLRESS, HILDA = 1123, 1194, 1225
RAPID_FIRE, DELIGHTFUL_KISS, EON_BLADE = 1092, 242, 243

CONFIG = {
 'MY_TYPE': 0,
 'BASICS': [MKANG, SLOWPOKE, SMOOCHUM, LATIAS, KYUREM, FEZANDIPITI, MEOWTH],
 'STAGE2S': set(),
 'MIN_BENCH': 3,
 'ABILITIES': {SLOWKING, FEZANDIPITI},
 'DRAW_ABILITIES': {SLOWKING, FEZANDIPITI},
 'EVOLVE_ORDER': [SLOWKING],
 'TRIGGER_ENERGY': set(),
 'ENERGY_TARGET_OVERRIDE': {},
 'KEEP_ACTIVE': {MKANG, LATIAS},
 'RETREAT_MIN_ENERGY': 3,
 'ATTACH_TARGETS': [MKANG, LATIAS, KYUREM, SLOWKING],
 'ENERGY_CAP': 3,
 'ATTACK_MIN_ENERGY': 3,
 'ENERGY_IDS': [PSY, TELEPATH],
 'FETCH_PRIORITY': [MKANG, SMOOCHUM, SLOWPOKE, PSY, TELEPATH, LATIAS, SLOWKING, KYUREM],
 # open Smoochum when it is in hand: its free attack loads the deck's scarce energy
 'SETUP_PRIORITY': [SMOOCHUM, MKANG, SLOWPOKE, LATIAS, KYUREM, MEOWTH, FEZANDIPITI],
 'PROMOTE_PRIORITY': [MKANG, LATIAS, KYUREM, SLOWKING, SMOOCHUM, SLOWPOKE],
 'DISCARD_PRIORITY': [CIPHER, COLRESS, HILDA, POKEPAD, ULTRABALL, LILLIE, SWITCH,
                      NIGHTSTRETCHER, PATCH, MEOWTH, FEZANDIPITI, ANNIHILAPE, CONKELDURR,
                      KYUREM, SMOOCHUM, SLOWPOKE, SLOWKING, TELEPATH, PSY, LATIAS, MKANG],
 'ENERGY_DISCARD': [PSY, TELEPATH],
 'RECOVERABLE': {MKANG, LATIAS, SLOWPOKE, SLOWKING, SMOOCHUM, KYUREM},
 'BENCH_FILLERS': set(),
 'HAND_IS_AMMO': False,
 'READY_ATTACKERS': {MKANG, LATIAS},
 'ATTACKER_LINE': {MKANG, LATIAS, KYUREM},
 'SETUP_RANK_MAX': 5,
 'COLLAPSE_BENCH': 2,
 'LILLIES_MAX_HAND': 5,
 'BOSS_KO_MAX': 0,
...
def PLAY_PRIORITY(cid, c):
    if c.get('hard_low') and cid in (POKEPAD, ULTRABALL, CIPHER): return 99
    if cid == MKANG and not c.get('mkang_out'): return 0
    if cid in (ULTRABALL, POKEPAD) and not c.get('mkang_out'): return 1
    if cid == SMOOCHUM and c.get('energy_hungry') and c['n_bench'] < 5: return 2
    if cid == PRIMECATCHER and c['has_attacks'] and c.get('gust_target'): return 3
    if cid in (SLOWPOKE, LATIAS, KYUREM, MEOWTH, FEZANDIPITI) and c['n_bench'] < 4: return 3
    if cid == ACADEMY and not c.get('our_stadium'): return 4
    if cid == PATCH and c.get('energy_hungry'): return 4
    if cid == SWITCH and c.get('want_switch'): return 4
    if cid == NIGHTSTRETCHER and c.get('pokes_in_discard', 0) >= 1: return 5
    if cid == COLRESS and c.get('energy_hungry'): return 5
    if cid == LILLIE and len(c['hand']) <= CONFIG['LILLIES_MAX_HAND']: return 6
    if cid == HILDA: return 7
    if cid == CIPHER: return 8
    if cid == ULTRABALL and len(c['hand']) >= 5: return 9
...
def effective_damage(obs, aid):
    ...
    if name == "Rapid-Fire Combo":
        dmg = 250
    elif name == "Delightful Kiss":
        dmg = 0
    elif name == "Myriad Leaf Shower":
        my_a = me(obs).active
        op_a = opp(obs).active
...
def _mkang_pick_attack(obs, attacks):
    """Energy-first attack rule. The deck runs 8 energy; Delightful Kiss (free, 0 damage)
    fetches 2 and is the only real accelerator, so take it whenever our attackers are short
    of Rapid-Fire Combo's 3-energy cost and nothing lethal is on offer."""
    try:
        by = {}
        for i, o in attacks:
        ...
        op = opp(obs).active
        hp = (op[0].hp if op and op[0] else 999) or 999
        # lethal first, always
        for aid, i in by.items():
            if effective_damage(obs, aid) >= hp and aid != DELIGHTFUL_KISS:
                return i
        starving = any(p.id in (MKANG, LATIAS) and energy_count(p) < 3 for p in my_in_play(obs))
        no_energy_in_hand = not any(c.id in cfg('ENERGY_IDS', []) for c in (me(obs).hand or []))
        if DELIGHTFUL_KISS in by and starving and no_energy_in_hand:
            return by[DELIGHTFUL_KISS]
        if RAPID_FIRE in by:
            return by[RAPID_FIRE]
    except Exception:
        pass
    return None
...
def choose_main(obs):
    ...
    if attacks:
        _mp = _mkang_pick_attack(obs, attacks)
        if _mp is not None:
            return [_mp]
    ...
    ctx = {
        ...
        'deck_count': my.deckCount, 'obs': obs,
        'our_stadium': bool(st.stadium and st.stadium[0] and st.stadium[0].id == ACADEMY),
        ...
        'mkang_out': MKANG in _pids,
        'energy_hungry': (not any(h in cfg('ENERGY_IDS', []) for h in _hids)
                          and any(p.id in (MKANG, LATIAS) and energy_count(p) < 3
                                  for p in _board)),
        'gust_target': any((b.hp or 999) <= 260 for b in (opp(obs).bench or []) if b),
        'want_switch': (SWITCH in _hids and my.active and my.active[0]
                        and my.active[0].id not in (MKANG, LATIAS)
                        and any(b is not None and b.id == MKANG and energy_count(b) >= 3
                                for b in (my.bench or []))),
    }
...
def dynamic_fetch(obs):
    ...
    pri = []
    if n_bench < 3:
        pri += [SLOWPOKE, SMOOCHUM, LATIAS]
    if not any(h in cfg('ENERGY_IDS', []) for h in hand_ids):
        pri += [SMOOCHUM, PSY, TELEPATH]
...
def choose_cards(obs):
    ...
    if ctx == SelectContext.TO_BENCH:
        ...
        return pick_by_priority(obs, select, [SLOWPOKE, SMOOCHUM, LATIAS] + fetch,
    ...
    if ctx in (SelectContext.REMOVE_DAMAGE_COUNTER, SelectContext.HEAL):
        ...
        def rm_dmg_score(i):
            ...
            return (1 if cid == MKANG else 0, dmg, hp)
