"""Rules agent oger_pal — faithful clone of palsystem (#8, 1151.6): his exact 60 + his mined
playbook (126 games, 78-48 = 61.9%, one list).

ENGINE (decoded from his replays): Meganium PASSIVELY DOUBLES every energy card (energies/
energyCards == 2.0 in 100% of observations; 68% WR with it in play vs 20% without). Hydrapple ex
(330hp) is the WIN CONDITION — Syrup Storm costs 2 colourless and deals 30 + 30 per GRASS energy
across ALL your Pokemon, so it hits a mean 343 while holding only 1-2 itself (183 attacks, 247
prizes, 72% KO rate). Teal Mask Ogerpon ex is the BATTERY/pivot: Teal Dance fires from the BENCH
(73% of 887 uses) and he wastes one on only 39/769 turns. Retreat costs exactly 1 energy card and
preserves the rest (127/127), so rotation is free tempo; 55/127 Ogerpon retreats swap INTO
Hydrapple. KPI: >=2.4 prizes by end of T8, differential >=+1.0; stranded energy on KO'd bodies
<=3/game (his wins 2.97 vs losses 7.00 — strongest loss predictor).
"""
...
def effective_damage(obs, aid):
    ...
    if name == "Syrup Storm":
        # 30 + 30 per GRASS energy across ALL our Pokemon (Meganium doubles each card,
        # and .energies already reports doubled UNITS). Mean 343 damage in his games.
        units = sum(len(p.energies or []) for p in my_in_play(obs))
        dmg = 30 + 30 * units
...
def choose_main(obs):
    ...
    ctx = {
        ...
        'our_stadium': bool(st.stadium and st.stadium[0] and st.stadium[0].id == FOREST),
        ...
        # --- palsystem specifics ---
        # the two engine pieces decide the game: Meganium by T6 = 71% WR, Hydrapple by T6 = 80%
        'need_engine': not (MEGANIUM in _pids and HYDRAPPLE in _pids),
        'need_basic': n_bench < 4,
        # Lana's Aid heals; he saves it for a big damaged body
        'hurt_big': any((p.maxHp or 0) >= 210 and (p.maxHp or 0) - (p.hp or 0) >= 100
                        for p in _board),
        # Boss gusts the SUPPORT bench: 53/59 undamaged, 33/59 zero energy
        'boss_target': any(not (b.energies or []) and (b.hp or 0) == (b.maxHp or 0)
                           for b in (opp(obs).bench or []) if b),
    }
