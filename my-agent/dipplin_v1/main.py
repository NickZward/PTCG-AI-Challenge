...
CONFIG = {
 ...
 'SETUP_PRIORITY': [GROOKEY, APPLIN, RELLOR, SHAYMIN, GENESECT],
 'PROMOTE_PRIORITY': [DIPPLIN, APPLIN, THWACKEY, GROOKEY, RABSCA, RELLOR, GENESECT, SHAYMIN],
 'DISCARD_PRIORITY': [1223, 1187, 1174, 1175, 1094, 1152, 1097, 1184, 1211, 1191,
                      1086, 1227, 1182, FESTIVAL, 1080, 1,
                      RELLOR, RABSCA, GENESECT, SHAYMIN, GROOKEY, APPLIN, THWACKEY, DIPPLIN],
 'ENERGY_DISCARD': [1],
...
def PLAY_PRIORITY(cid, c):
    ...
    if c['n_bench'] < 2 and not any(x.id in (APPLIN, GROOKEY, RELLOR, SHAYMIN, GENESECT) for x in c['hand']):
    ...
    if c.get('energy_drought'):
        ...
        if cid == 1184 and c.get('energy_in_discard'): return 1
