...
CONFIG = {
 'MY_TYPE': 5,
 'BASICS': [741, 305, 343, 140],
 'STAGE2S': {743},
 'MIN_BENCH': 3,
 'ABILITIES': {66, 140},
 'DRAW_ABILITIES': {66, 140},
 'EVOLVE_ORDER': [743, 742, 66],
 'TRIGGER_ENERGY': {13, 19},
 'ENERGY_TARGET_OVERRIDE': {13: [343, 305, 66, 741]},
 'KEEP_ACTIVE': {743},
 'RETREAT_MIN_ENERGY': 1,
 'ATTACH_TARGETS': [743, 742, 741, 66],
 'ENERGY_CAP': 1,
 'FETCH_PRIORITY': [743, 742, 741, 1079, 66, 305, 5, 19, 13, 343, 140, 1231, 1225],
 'SETUP_PRIORITY': [741, 305, 343, 140],
 'PROMOTE_PRIORITY': [743, 66, 742, 741, 305, 343, 140],
 'DISCARD_PRIORITY': [1266, 1184, 1129, 1097, 1081, 1197, 1182, 5, 1152, 1086, 1225, 1231, 19, 13, 1079, 305, 343, 741, 742, 66, 140, 743],
 'ENERGY_DISCARD': [5, 19, 13],
 'RECOVERABLE': {741, 742, 743, 305, 66, 343, 140},
 'BENCH_FILLERS': {1086},
 'HAND_IS_AMMO': True,
 'READY_ATTACKERS': {743},
 'SETUP_RANK_MAX': 5,
...
def PLAY_PRIORITY(cid, c):
    if cid == 1129 and c.get('deck_count', 60) < 15 and c.get('pokes_in_discard', 0) >= 2:
        return 0   # Sacred Ash: anti-mill/deck-out refuel — +5 cards AND +5 bodies
    if c.get('hard_low') and cid in (1231, 1225, 1152, 1086, 1122, 1097, 1184):
        return 99
    if c.get('deck_low') and cid in (1122, 1097, 1184):
        return 99
    if c.get('pokes_in_discard', 0) >= 2 and cid in (1097, 1129, 1184):
        return 3   # recovery engine: bodies are down, bring them home
    if cid in (741, 305) and c['n_bench'] < 5: return 0
    if cid == 1079 and c['stage2_in_hand']: return 1
    if cid == 1231: return 2
    if cid == 1225 and not c['have'](743): return 3
    if cid == 1086 and c['n_bench'] < 5: return 4
    if cid == 1152 and not c['have'](743): return 5
    if cid == 1081: return 6
    if cid == 343 and c['n_bench'] < 5: return 7
    if cid == 1197: return 8
    if cid == 1182 and c['has_attacks'] and c['opp_hp'] <= 20*len(c['hand']): return 9
    if cid == 140 and c['n_bench'] < 5: return 10
    if cid == 1097: return 11
    if cid == 1129: return 12
    if cid == 1184: return 13
    if cid == 1266: return 14
    if cid == 1182: return 15
    return 99
# ===================================================================
