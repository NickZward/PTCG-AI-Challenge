"""Gauntlet pilot: Okidogi Fighting aggro (miruto's list). Okidogi(116) Good
Punch {F}{F} 70 (+100 w/ Dark via Prism = Adrena-Power). Basic attacker, fast."""
import os, random
from cg.api import (Observation, to_observation_class, AreaType, OptionType, SelectType, SelectContext)
OKIDOGI=116; SOLROCK=676; LUNATONE=675; BINACLE=1051; BARBARACLE=1052
GROOKEY, THWACKEY = OKIDOGI, OKIDOGI
APPLIN, DIPPLIN = OKIDOGI, OKIDOGI      # Okidogi is a Basic attacker (no evolve)
RELLOR, RABSCA = BINACLE, BARBARACLE
GENESECT, SHAYMIN = -1, -2
FESTIVAL = 1264                          # Battle Cage
CONFIG = {
 'MY_TYPE': 6,
 'BASICS': [OKIDOGI, SOLROCK, LUNATONE, BINACLE],
 'STAGE2S': set(),
 'MIN_BENCH': 3,
 'ABILITIES': {OKIDOGI, SOLROCK, LUNATONE},
 'DRAW_ABILITIES': set(),
 'EVOLVE_ORDER': [BARBARACLE],
 'TRIGGER_ENERGY': {16},
 'ENERGY_TARGET_OVERRIDE': {},
 'KEEP_ACTIVE': {OKIDOGI},
 'RETREAT_MIN_ENERGY': 2,
 'ATTACH_TARGETS': [OKIDOGI, BARBARACLE, SOLROCK],
 'ENERGY_CAP': 2,
 'ENERGY_IDS': [6, 16],
 'FETCH_PRIORITY': [OKIDOGI, 6, 16, SOLROCK, LUNATONE, BINACLE, BARBARACLE, 1142, 1152, 1182],
 'SETUP_PRIORITY': [OKIDOGI, SOLROCK, LUNATONE, BINACLE],
 'PROMOTE_PRIORITY': [OKIDOGI, BARBARACLE, SOLROCK, LUNATONE],
 'DISCARD_PRIORITY': [1264, 1142, 1152, 1097, 1194, 1213, 1238, 1227, 1182, 6, 16, SOLROCK, LUNATONE, BINACLE, BARBARACLE, OKIDOGI],
 'ENERGY_DISCARD': [6, 16],
 'RECOVERABLE': {OKIDOGI, BINACLE, BARBARACLE},
 'BENCH_FILLERS': set(),
 'HAND_IS_AMMO': False,
 'READY_ATTACKERS': {OKIDOGI},
 'SETUP_RANK_MAX': 5,
}
def PLAY_PRIORITY(cid, c):
    if cid in (OKIDOGI, SOLROCK, LUNATONE, BINACLE) and c['n_bench'] < 4: return 0
    if cid == 1142: return 1                              # Fighting Gong: search energy/basic
    if cid == 1182 and c['has_attacks'] and c['opp_hp'] <= 170: return 2   # Boss for KO
    if cid == 1264 and not c.get('our_stadium'): return 3
    if cid == 1152: return 4
    if cid == 1238: return 5                              # Tarragon: recover energy/pokemon
    if cid == 1227 and len(c['hand']) <= 5: return 6
    if cid == 1194 and len(c['hand']) <= 5: return 6      # Colress's
    if cid == 1213 and c.get('opp_hand',0) >= 6: return 6 # Judge
    if cid == 1097 and c.get('pokes_in_discard',0) >= 1: return 7
    return 99
