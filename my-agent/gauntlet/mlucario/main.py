"""Gauntlet pilot: Mega Lucario (mined field list, 2-1)."""
import os, random
from cg.api import (Observation, to_observation_class, AreaType, OptionType, SelectType, SelectContext)
GROOKEY, THWACKEY = 673, 674        # Makuhita -> Hariyama
APPLIN, DIPPLIN = 677, 678          # Riolu -> Mega Lucario ex
RELLOR, RABSCA = 675, 676           # Lunatone / Solrock
GENESECT, SHAYMIN = -1, -2
FESTIVAL = 1252                     # Gravity Mountain
CONFIG = {
 'MY_TYPE': 4,
 'BASICS': [677, 673, 675, 676],
 'STAGE2S': set(),
 'MIN_BENCH': 2,
 'ABILITIES': {675, 676, 678},
 'DRAW_ABILITIES': set(),
 'EVOLVE_ORDER': [678, 674],
 'TRIGGER_ENERGY': set(),
 'ENERGY_TARGET_OVERRIDE': {},
 'KEEP_ACTIVE': {678},
 'RETREAT_MIN_ENERGY': 2,
 'ATTACH_TARGETS': [678, 677, 674, 673],
 'ENERGY_CAP': 2,
 'FETCH_PRIORITY': [678, 677, 6, 674, 673, 675, 676, 1152, 1227],
 'SETUP_PRIORITY': [677, 673, 675, 676],
 'PROMOTE_PRIORITY': [678, 674, 676, 675, 677, 673],
 'DISCARD_PRIORITY': [1102, 1123, 1142, 1141, 1152, 1192, 1227, 1197, 1182, 6, 673, 675, 676, 677, 674, 678],
 'ENERGY_DISCARD': [6],
 'RECOVERABLE': {677, 678, 674, 673},
 'BENCH_FILLERS': set(),
 'HAND_IS_AMMO': False,
 'READY_ATTACKERS': {678},
 'SETUP_RANK_MAX': 5,
...
def PLAY_PRIORITY(cid, c):
    if cid in (677, 673, 675, 676) and c['n_bench'] < 4: return 0
    if cid == 1142: return 1
    if cid == 1182 and c['has_attacks'] and c['opp_hp'] <= 270: return 2
    if cid == 1252 and not c.get('our_stadium'): return 3
    if cid == 1141 and c['has_attacks']: return 4
    if cid == 1102: return 5
    if cid == 1152: return 6
    if cid == 1159: return 7
    if cid == 1192 and len(c['hand']) <= 5: return 8
    if cid == 1227 and len(c['hand']) <= 5: return 8
    if cid == 1197: return 9
    if cid == 1123: return 12
    return 99
