"""Gauntlet pilot: Crustle Wall (mined field list, 4-1)."""
import os, random
from cg.api import (Observation, to_observation_class, AreaType, OptionType, SelectType, SelectContext)
GROOKEY, THWACKEY = 756, 756        # Mega Kangaskhan
APPLIN, DIPPLIN = 344, 345          # Dwebble -> Crustle
RELLOR, RABSCA = 117, 117           # Ogerpon
...
CONFIG = {
 ...
 'ATTACH_TARGETS': [345, 756, 344, 117],
 'ENERGY_CAP': 2,
 'FETCH_PRIORITY': [345, 344, 756, 14, 18, 20, 11, 1086, 1122, 1227],
 'SETUP_PRIORITY': [344, 756, 117],
 'PROMOTE_PRIORITY': [345, 756, 117, 344],
 'DISCARD_PRIORITY': [1122, 1147, 1123, 1121, 1086, 1225, 1227, 1197, 1182, 1, 117, 344, 756, 345],
 'ENERGY_DISCARD': [1, 11, 14, 18, 20],
 'RECOVERABLE': {344, 345, 756},
 'BENCH_FILLERS': {1086},
 'HAND_IS_AMMO': False,
 'READY_ATTACKERS': {345, 756},
 'SETUP_RANK_MAX': 5,
}
def PLAY_PRIORITY(cid, c):
    if cid in (344, 756, 117) and c['n_bench'] < 4: return 0
    if cid == 1086 and c['n_bench'] < 4: return 1
    if cid == 1264 and not c.get('our_stadium'): return 2
    if cid == 1182 and c['has_attacks'] and c['opp_hp'] <= 150: return 3
    if cid == 1121: return 4
    if cid == 1147: return 5
    if cid == 1159: return 6
    if cid == 1225 and len(c['hand']) <= 5: return 7
    if cid == 1227 and len(c['hand']) <= 5: return 7
