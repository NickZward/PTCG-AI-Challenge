"""Gauntlet pilot: Mega Starmie (mined field list, 10-9)."""
import os, random
from cg.api import (Observation, to_observation_class, AreaType, OptionType, SelectType, SelectContext)
GROOKEY, THWACKEY = 666, 666        # Cinderace
APPLIN, DIPPLIN = 1030, 1031        # Staryu -> Mega Starmie ex
RELLOR, RABSCA = -3, -4
...
CONFIG = {
 ...
 'ATTACH_TARGETS': [1031, 1030, 666],
 'ENERGY_CAP': 3,
 'FETCH_PRIORITY': [1031, 1030, 3, 17, 666, 1086, 1145, 1122, 1227],
 'SETUP_PRIORITY': [1030, 666],
 'PROMOTE_PRIORITY': [1031, 666, 1030],
 'DISCARD_PRIORITY': [1122, 1120, 1145, 1121, 1086, 1223, 1225, 1229, 1189, 1227, 1182, 3, 17, 666, 1030, 1031],
 'ENERGY_DISCARD': [3, 17],
 'RECOVERABLE': {1030, 1031, 666},
 'BENCH_FILLERS': {1086},
 'HAND_IS_AMMO': False,
 'READY_ATTACKERS': {1031},
 'SETUP_RANK_MAX': 5,
}
def PLAY_PRIORITY(cid, c):
    if cid in (1030, 666) and c['n_bench'] < 4: return 0
    if cid == 1086 and c['n_bench'] < 4: return 1
    if cid == 1145: return 2
    if cid == 1182 and c['has_attacks'] and c['opp_hp'] <= 200: return 3
    if cid == 1189 and len(c['hand']) <= 6: return 4
    if cid == 1121: return 5
    if cid == 1159: return 6
    if cid == 1229 and len(c['hand']) <= 5: return 7
    if cid == 1227 and len(c['hand']) <= 5: return 7
    if cid == 1120: return 8
    if cid == 1223 and len(c['hand']) <= 2: return 9
