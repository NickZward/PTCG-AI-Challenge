"""Rules agent mluc_v1 — Majkel1337's Mega Lucario ex (his exact 60, one list across all
461 of his games). Playbook mined from 308 winning + 152 losing games of the #1 player.

Core loop: Riolu -> M-Lucario ex (340hp) by turn 2, then alternate Mega Brave 270 (locked out
the following turn) with Aura Jab 130 (which re-attaches up to 3 Fighting Energy from the
discard to the BENCH, charging the 2nd Lucario and the Hariyama). Solrock+Lunatone is a real
prize engine (Cosmic Beam 70 kills <=70hp starters, 172/175, 37% of his first-KOs); Lunatone's
ability (discard 1, draw 3) is the draw engine; Hariyama (Wild Press 210, 88% KO rate) is the
designated successor attacker. KPI: Mega Braves per game 0->33% WR, 2->91%. Prize tempo, not
lethal: nobody ever takes 6 prizes; the prize leader at the turn cap wins 92% of the time.
"""
...
MLUCARIO, RIOLU, SOLROCK, LUNATONE, MAKUHITA, HARIYAMA = 678, 677, 676, 675, 673, 674
FIGHT = 6
ULTRABALL, POWERPRO, GONG, POKEPAD, JUDGE = 1121, 1141, 1142, 1152, 1213
LILLIE, SWITCH, BOSS, WALLY, CAPE = 1227, 1123, 1182, 1229, 1159
MEGA_BRAVE, AURA_JAB, COSMIC_BEAM, WILD_PRESS = 983, 982, 980, 978

CONFIG = {
 'MY_TYPE': 0,
 'BASICS': [SOLROCK, RIOLU, LUNATONE, MAKUHITA],
 'STAGE2S': set(),
 'MIN_BENCH': 3,
 'ABILITIES': {LUNATONE},          # discard 1 -> draw 3; the whole draw engine
 'DRAW_ABILITIES': {LUNATONE},
 'EVOLVE_ORDER': [MLUCARIO, HARIYAMA],
 'TRIGGER_ENERGY': set(),
 'ENERGY_TARGET_OVERRIDE': {},
 'KEEP_ACTIVE': {MLUCARIO, HARIYAMA},   # everything else should hand off
 'RETREAT_MIN_ENERGY': 1,          # Aura Jab needs only 1 — unstick earlier
 'ATTACH_TARGETS': [MLUCARIO, RIOLU, HARIYAMA, SOLROCK, MAKUHITA],
 'ENERGY_CAP': 2,                  # active M-Lucario is HARD-CAPPED at 2 (63% of turn-ends)
 'ATTACK_MIN_ENERGY': 2,
 'ENERGY_IDS': [FIGHT],
 'FETCH_PRIORITY': [MLUCARIO, RIOLU, FIGHT, LUNATONE, HARIYAMA, SOLROCK, MAKUHITA],
 # 461/461 games, zero exceptions: Solrock > Riolu > Lunatone > Makuhita
 # DEVIATION (probe-driven): Lunatone demoted to LAST — as our active it locks
 # the turn (0 energy, no attack, no retreat). He can escape it; we cannot.
 'SETUP_PRIORITY': [SOLROCK, RIOLU, MAKUHITA, LUNATONE],
 'PROMOTE_PRIORITY': [MLUCARIO, HARIYAMA, SOLROCK, RIOLU, LUNATONE, MAKUHITA],
 # Ultra Ball costs: he pitches spare supporters/Solrock, NEVER the last Riolu/Lucario
 'DISCARD_PRIORITY': [JUDGE, SOLROCK, MAKUHITA, LILLIE, POWERPRO, ULTRABALL, GONG,
                      POKEPAD, SWITCH, BOSS, WALLY, CAPE, LUNATONE, HARIYAMA,
                      FIGHT, RIOLU, MLUCARIO],
 'ENERGY_DISCARD': [FIGHT],
 'RECOVERABLE': {MLUCARIO, RIOLU, SOLROCK, LUNATONE, MAKUHITA, HARIYAMA},
 'BENCH_FILLERS': set(),
 'HAND_IS_AMMO': False,
 'READY_ATTACKERS': {MLUCARIO},
 'ATTACKER_LINE': {RIOLU, MLUCARIO},
 'SETUP_RANK_MAX': 5,
 'COLLAPSE_BENCH': 2,
 'LILLIES_MAX_HAND': 5,
 'BOSS_KO_MAX': 0,
 'BOSS_BENCH_MAX': 130,            # he gusts UNDEVELOPED <=110hp support, then Aura Jabs it
 'ROTATE_HP': 0,
 'WALLY_HEAL_AT': 140,             # Wally at <=60% of 340 = ~204 dmg... use dmg>=140 proxy
 'RACE_DECK_AT': 15,
 'RACE_MARGIN': 3,
 'DECK_LOW_AT': 6,
 'XEROSIC_HAND': 99,
...
def PLAY_PRIORITY(cid, c):
    if c.get('hard_low') and cid in (POKEPAD, GONG, ULTRABALL, JUDGE): return 99
    # 11. Wally FIRST action of the turn, on a damaged M-Lucario only (178/178)
    if cid == WALLY and c.get('ml_dmg', 0) >= CONFIG['WALLY_HEAL_AT']: return 0
    # 2. get M-Lucario down on turn 2: searches that find the line outrank everything
    if cid == ULTRABALL and c.get('need_ml'): return 1
    if cid == POKEPAD and c.get('need_riolu'): return 2
    if cid == GONG and c.get('need_basic'): return 2
    # bench out the basics early (he benches every basic offered, 55/55)
    if cid == LUNATONE and not c.get('lunatone_out'): return 1
    if cid in (POKEPAD, ULTRABALL, GONG) and not c.get('lunatone_out') \
            and c.get('lunatone_available'): return 1
    if cid in (SOLROCK, RIOLU, LUNATONE, MAKUHITA) and c['n_bench'] < 5: return 3
    # 13. Hero's Cape T1 on the Riolu that becomes the main Lucario (+100hp -> 440)
    if cid == CAPE and c.get('cape_want'): return 4
    # 12. Boss an undeveloped zero-energy small bench mon, then Aura Jab it
    if cid == BOSS and c['has_attacks'] and c.get('boss_target'): return 5
    # 8. Power Pro ONLY to cross a KO threshold (stacks +30)
    if cid == POWERPRO and c.get('powerpro_converts'): return 6
    # 1 draw supporter on 60-70% of turns 2-5; Lillie early, Judge later
    if cid == LILLIE and len(c['hand']) <= CONFIG['LILLIES_MAX_HAND']: return 7
    if cid == JUDGE and len(c['hand']) <= 3: return 8
    if cid == SWITCH and c.get('want_switch'): return 2   # free handoff to the 270
    if cid == GONG and c.get('energy_hungry'): return 3   # his T3+ Gong is 67-76% energy
    if cid == GONG: return 9
    if cid == POKEPAD: return 10
    if cid == ULTRABALL and len(c['hand']) >= 4: return 11
    return 99
# ===================================================================
...
def effective_damage(obs, aid):
    ...
    if name == "Cosmic Beam":
        # does NOTHING without Lunatone benched
        if not any(b is not None and b.id == LUNATONE for b in (me(obs).bench or [])):
            dmg = 0
    return dmg * opp_weak_mult(obs)


def _mluc_pick_attack(obs, attacks):
    """Majkel's measured attack rule (n=764 both-legal decisions):
      * Aura Jab ALSO lethal  -> Aura Jab (61%) — it kills AND keeps Mega Brave off lockout
      * only Mega Brave lethal -> Mega Brave (92%)
      * neither lethal         -> Mega Brave (82%)
    Cosmic Beam is taken only vs small targets (172 KOs/175 vs <=70hp; 6/35 vs >110hp).
    Returns an option index or None to fall through to the generic chooser."""
    try:
        op = opp(obs).active
        if not op or not op[0]:
            return None
        hp = op[0].hp or 999
        mx = op[0].maxHp or 999
        boost = 0   # was 30*in-hand Power Pros: the boost was never actually played before the returned attack resolved — phantom lethality
        by = {}
        for i, o in attacks:
            if o.attackId is not None:
                by.setdefault(o.attackId, i)
        mb, aj = by.get(MEGA_BRAVE), by.get(AURA_JAB)
        cb, wp = by.get(COSMIC_BEAM), by.get(WILD_PRESS)
        mult = opp_weak_mult(obs)
        if aj is not None and (130 + boost) * mult >= hp:
            return aj                                  # cheapest lethal; preserves Mega Brave
        if mb is not None and (270 + boost) * mult >= hp:
            return mb
        if cb is not None and mx <= 110 and (70 + boost) * mult >= hp:
            # only while the Lucario engine isn't online yet — he hands off to Aura Jab
            # after a Cosmic Beam 45.5% of the time
            if not any(b is not None and b.id == MLUCARIO and energy_count(b) >= 2
                       for b in (me(obs).bench or [])):
                return cb                              # the turn-1/2 free prize
        if wp is not None and (210 + boost) * mult >= hp:
            return wp
        # NEITHER LETHAL: fall through to the trainer phase (PLAY_PRIORITY). The old
        # unconditional 'return mb' here made the pilot attack as its FIRST main action on
        # every Lucario turn, so Wally/Boss/searches/draws below never played — Majkel plays
        # a supporter AND attacks on 60-70% of turns; attacks still happen at the ladder's end.
    except Exception:
        pass
    return None


def _powerpro_in_hand(obs):
    return sum(1 for c in (me(obs).hand or []) if c.id == POWERPRO)
...
def choose_main(obs):
    ...
    def _energy_attach():
        ...
        def score(o):
            ...
            if cid not in att_pri:
                arm = 5
            elif cid == MLUCARIO and is_active and en == 0:
                arm = -1                     # dry active Mega = dead turn; feed it first
    ...
    # 3.14 BRING IN THE CHARGED LUCARIO (round 5). If a benched M-Lucario can Mega Brave
    # (>=2 energy) and the current active cannot (dry, or locked out, or isn't a Lucario),
    # retreat into it. This is the step that converts Jab-recursion energy into 270 damage.
    if not st.retreated:
        _r2 = next((i for i, o in enumerate(select.option)
                    if o.type == OptionType.RETREAT), None)
        if _r2 is not None:
            _act = my.active[0] if my.active else None
            _bench_ready = any(b is not None and b.id == MLUCARIO and energy_count(b) >= 2
                               for b in (my.bench or []))
            if _bench_ready and _act is not None:
                _act_can_mb = (_act.id == MLUCARIO and energy_count(_act) >= 2
                               and any(o.attackId == MEGA_BRAVE for _, o in attacks))
                if not _act_can_mb:
                    return [_r2]

    # 3.15 NEVER-STALL HANDOFF (round 3; his attack rate is 81%, ours was 51%).
    # If the ACTIVE has no legal attack this turn but a benched body is charged enough to
    # attack, retreat into it. Costs: M-Lucario retreat 2, Solrock/Lunatone 1.
    if not attacks and not st.retreated:
        _ridx = next((i for i, o in enumerate(select.option)
                      if o.type == OptionType.RETREAT), None)
        if _ridx is not None:
            def _can_attack(p):
                if p is None: return False
                e = energy_count(p)
                return ((p.id == MLUCARIO and e >= 1)      # Aura Jab needs 1
                        or (p.id == HARIYAMA and e >= 3)   # Wild Press needs 3
                        or (p.id == SOLROCK and e >= 1
                            and any(b is not None and b.id == LUNATONE
                                    for b in (my.bench or []))))
            if any(_can_attack(b) for b in (my.bench or [])):
                return [_ridx]
            _act0 = my.active[0] if my.active else None
            if (_act0 is not None and energy_count(_act0) == 0
                    and any(b is not None and energy_count(b) >= 1 for b in (my.bench or []))):
                return [_ridx]
    ...
    if attacks:
        _mp = _mluc_pick_attack(obs, attacks)
        if _mp is not None:
            return [_mp]
    ...
    _board = my_in_play(obs)
    _pids = [p.id for p in _board]
    _hids = [c.id for c in hand]
    _ml = [p for p in _board if p.id == MLUCARIO]
    _opp_bench = [b for b in (opp(obs).bench or []) if b]
    ctx = {
        ...
        'stage2_in_hand': False,
        'deck_low': deck_low, 'hard_low': hard_low,
        'pokes_in_discard': sum(1 for c in (my.discard or []) if c.id in cfg('RECOVERABLE', set())),
        'deck_count': my.deckCount, 'obs': obs, 'our_stadium': False,
        'opp_bench': len(_opp_bench),
        'opp_bench_min_hp': min([(b.hp or 999) for b in _opp_bench] or [999]),
        'deck_race': deck_race, 'opp_hand': opp(obs).handCount or 0,
        'opp_is_ex': bool(opp_ci is not None and getattr(opp_ci, 'ex', False)),
        'energy_drought': (sum(len(p.energies or []) for p in _board) == 0
        ...
        # ---- M-Lucario specific ----
        'ml_dmg': max([((p.maxHp or 0) - (p.hp or 0)) for p in _ml] or [0]),
        'lunatone_out': LUNATONE in _pids,
        'lunatone_available': True,
        # any Lucario/Hariyama short of its attack threshold and no energy in hand
        'energy_hungry': (not any(h in cfg('ENERGY_IDS', []) for h in _hids)
                          and any((p.id == MLUCARIO and energy_count(p) < 2)
                                  or (p.id == HARIYAMA and energy_count(p) < 3)
                                  for p in _board)),
        'need_ml': (MLUCARIO not in _pids and MLUCARIO not in _hids
                    and (RIOLU in _pids or RIOLU in _hids)),
        'need_riolu': (RIOLU not in _pids and RIOLU not in _hids and MLUCARIO not in _pids),
        'need_basic': (n_bench < 3 or not any(h in cfg('ENERGY_IDS', []) for h in _hids)),
        'cape_want': (CAPE in _hids and any(p.id in (RIOLU, MLUCARIO) and not p.tools
                                            for p in _board)),
        # gust an UNDEVELOPED, zero-energy, small bench mon (45% <=110hp, 68% no energy)
        'boss_target': any((b.maxHp or 999) <= 130 and not (b.energies or [])
                           for b in _opp_bench),
        # Power Pro only when +30 crosses the kill line
        'powerpro_converts': (POWERPRO in _hids and attacks and 0 < opp_hp
                              and any((130 if o.attackId == AURA_JAB else
                                       270 if o.attackId == MEGA_BRAVE else 0)
                                      + 30 * _powerpro_in_hand(obs) >= opp_hp >
                                      (130 if o.attackId == AURA_JAB else
                                       270 if o.attackId == MEGA_BRAVE else 0)
                                      for _, o in attacks)),
        # lockout turn with a charged spare Lucario on the bench -> Switch into it
        'want_switch': (SWITCH in _hids
                        and any(b is not None and b.id == MLUCARIO and energy_count(b) >= 2
                                for b in (my.bench or []))
                        and not (my.active and my.active[0] and my.active[0].id == MLUCARIO
                                 and energy_count(my.active[0]) >= 2
                                 and any(o.attackId == MEGA_BRAVE for _, o in attacks))),
    }
...
def oracle_attacks(obs):
    ...
    try:
        ...
        pool += [FIGHT] * max(0, need - len(pool))   # was GRASS (undefined) — NameError killed oracle_attacks on every call, swallowed by the blanket except
...
def dynamic_fetch(obs):
    """Need-aware search: the whole plan is M-Lucario on turn 2, then a 2nd on the bench."""
    my = me(obs)
    ...
    pri = []
    n_ml = play_ids.count(MLUCARIO) + hand_ids.count(MLUCARIO)
    # 1. the Mega and its basic — first one by turn 2, second one on the bench
    if n_ml < 2 and (RIOLU in play_ids or RIOLU in hand_ids):
        pri.append(MLUCARIO)
    if play_ids.count(RIOLU) + hand_ids.count(RIOLU) + play_ids.count(MLUCARIO) < 2:
        pri.append(RIOLU)
    # 2. Lunatone = draw engine AND Cosmic Beam enabler (benched in 74% of his turns;
    #    his T1 Poke Pad fetches Lunatone 41% — ENGINE BEFORE ATTACKER on turn 1)
    if LUNATONE not in play_ids:
        pri.insert(0, LUNATONE)          # engine first — 74% of his turns have it benched
    if SOLROCK not in play_ids and SOLROCK not in hand_ids and n_bench < 4:
        pri.append(SOLROCK)
    # 3. energy when a Lucario is dry (Gong fetches energy 67-76% from T3)
    if not any(h in cfg('ENERGY_IDS', []) for h in hand_ids) and \
            any(p.id == MLUCARIO and len(p.energies or []) < 2 for p in in_play):
        pri.append(FIGHT)
    # 4. Hariyama = the successor attacker (Poke Pad fetches it 34% from T4+)
    if HARIYAMA not in play_ids and MAKUHITA in play_ids:
        pri.append(HARIYAMA)
    if MAKUHITA not in play_ids and MAKUHITA not in hand_ids and n_bench < 4:
        pri.append(MAKUHITA)
...
def choose_cards(obs):
    ...
    if ctx == SelectContext.TO_BENCH:
        ...
        return pick_by_priority(obs, select, [RIOLU, LUNATONE, SOLROCK, MAKUHITA] + fetch,
    ...
    if ctx in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
        ...
        def score(i):
            ...
            en = energy_count(c) if hasattr(c, "energies") else 0
            # PROMOTION (his data: 80/121 Wild Presses arrive by promotion after a KO —
            # he promotes the CHARGED successor, not a fresh dry Lucario). A 0-energy body in
            # the active spot is a dead turn: our probe found it in 6 of 13 dead turns.
            en_c = energy_count(c) if hasattr(c, 'energies') else 0
            if cid == MLUCARIO and en_c >= 2: tier = 0      # can Mega Brave immediately
            elif cid == HARIYAMA and en_c >= 3: tier = 0    # can Wild Press immediately
            elif cid == MLUCARIO and en_c >= 1: tier = 1    # can Aura Jab
            elif cid == SOLROCK and en_c >= 1: tier = 2     # Cosmic Beam chip
            elif en_c >= 1: tier = 3
            else: tier = 4                                   # dry bodies last
    ...
    if ctx in (SelectContext.ATTACH_FROM, SelectContext.ATTACH_TO, SelectContext.EFFECT_TARGET):
        # AURA JAB RECURSION (1,853/1,853 observed re-attachments went to the BENCH):
        # Hariyama 27% > bench M-Lucario 20% > Riolu 19% > Makuhita 19% > Solrock 11%.
        # This is the ONLY way the 2nd Lucario and the Hariyama ever get charged.
        _ml_ready = any(b is not None and b.id == MLUCARIO and energy_count(b) >= 2
                        for b in (me(obs).bench or []))
        order = ([HARIYAMA, MLUCARIO, RIOLU, MAKUHITA, SOLROCK] if _ml_ready
                 else [MLUCARIO, RIOLU, HARIYAMA, MAKUHITA, SOLROCK])
        def _jab_score(i):
            o = select.option[i]
            pi = o.playerIndex if o.playerIndex is not None else my_index
            c = card_at(obs, o.area, o.index, pi)
            cid = getattr(c, 'id', None)
            bench = 0 if o.area == AreaType.BENCH else 1        # bench first, always
            rank = order.index(cid) if cid in order else len(order)
            en = energy_count(c) if hasattr(c, 'energies') else 0
            return (bench, rank, en)
        idxs = sorted(range(len(select.option)), key=_jab_score)
    ...
    if ctx in (SelectContext.REMOVE_DAMAGE_COUNTER, SelectContext.HEAL):
        ...
        def rm_dmg_score(i):
            ...
            return (1 if cid == MLUCARIO else 0, dmg, hp)
