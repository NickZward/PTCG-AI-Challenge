    ...
    if ctx in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
        ...
        if opp_side and len(opp_side) == len([o for o in select.option if o.playerIndex is not None]):
            ...
            try:
                ...
                my_dmg = 180
            THREAT = {743: 3, 742: 2, 648: 3, 647: 2}   # evolved/attacker-line targets
            def gust_score(i):
                o = select.option[i]
...
# ============ AZ: learned POLICY + VALUE nets, policy-guided value search ============
_AZ = {"value": None, "policy": None, "vocab": None, "cf": None, "adm": None, "deck": None, "ok": None}

def _az_dirs():
    dirs = []
    try: dirs.append(os.path.dirname(os.path.abspath(__file__)))
    except NameError: pass
    dirs += [os.getcwd(), "/kaggle_simulations/agent", "."]
    seen = set(); out = []
    for d in dirs:
        if d and d not in seen: seen.add(d); out.append(d)
    return out

def _resolve_fixed(obs, opt):
...
def _az_load():
    if _AZ["ok"] is not None: return _AZ["ok"]
    import sys as _sys, joblib, json as _j
    for base in _az_dirs():
        try:
            vp = os.path.join(base, "value_net.joblib"); pp = os.path.join(base, "policy_net.joblib")
            vc = os.path.join(base, "bc_vocab.json")
            if not (os.path.exists(vp) and os.path.exists(pp) and os.path.exists(vc)): continue
            if base not in _sys.path: _sys.path.insert(0, base)
            import bc_features as F
            F.resolve_card_id = _resolve_fixed
            _AZ["value"] = joblib.load(vp); _AZ["policy"] = joblib.load(pp)
            vj = _j.load(open(vc)); _AZ["vocab"] = vj["vocab"]; _AZ["cf"] = vj["card_feats"]
            from cg.api import all_attack
            _AZ["adm"] = {str(a.attackId): (a.damage or 0) for a in all_attack()}
            _AZ["deck"] = read_deck_csv(); _AZ["ok"] = True; return True
        except Exception:
            continue
    _AZ["ok"] = False; return False

def _unseen_pool(obs):
    from collections import Counter as _C
    my = me(obs); pool = _C(_AZ["deck"]); seen = []
    seen += [c.id for c in (my.hand or [])] + [c.id for c in (my.discard or [])]
    for pkm in my_in_play(obs):
        seen.append(pkm.id); seen += [getattr(e, "id", None) for e in (pkm.energies or [])]
    st = obs.current.stadium
    if st and st[0] is not None: seen.append(st[0].id)
    for cid in seen:
        if cid is not None and pool.get(cid, 0) > 0: pool[cid] -= 1
    return list(pool.elements())

def _rule_dispatch(o):
    t = o.select.type
    if t == SelectType.MAIN: r = choose_main(o)
    elif t in (SelectType.CARD, SelectType.CARD_OR_ATTACHED_CARD, SelectType.ATTACHED_CARD): r = choose_cards(o)
    elif t == SelectType.YES_NO: r = choose_yes_no(o)
    elif t == SelectType.ATTACK: r = choose_attack(o)
    elif t == SelectType.COUNT: r = choose_count(o)
    elif t == SelectType.EVOLVE: r = choose_cards(o)
    elif t == SelectType.ENERGY: r = pick_by_priority(o, o.select, cfg('ENERGY_DISCARD', []), count=max(o.select.minCount, 1))
    else: r = safe_fallback(o.select)
    return _validate(r, o.select)

# ---- OPPONENT ARCHETYPE DETECTION (for realistic deep-search determinization) ----
# The DEPTH-2 rollout is only trustworthy if the simulated opponent has their REAL cards to play.
# We detect the opponent's archetype from revealed cards (IDF-weighted so distinctive attackers
# count, shared energy/staples don't), then fill their hidden zones from that decklist. If we can't
# identify them confidently we fall back to az0's safe DEPTH-1 (score end-of-our-turn only).
_ARCH = None
def _load_arch():
    global _ARCH
    ...
    for base in _az_dirs():
        try:
            if base not in _s.path: _s.path.insert(0, base)
            import arch_decks as _ad
            _ARCH = {"decks": _ad.ARCH_DECKS, "idf": _ad.CARD_IDF}
            break
        except Exception:
            continue
    return _ARCH

def _opp_revealed_ids(obs):
    op = opp(obs); seen = [c.id for c in (op.discard or [])]
...
def _detect_opp_deck(obs):
    """(name, 60-card decklist) of the confidently-matched archetype, else (None, None)."""
    A = _load_arch(); decks = A["decks"]; idf = A["idf"]
    if not decks: return None, None
    from collections import Counter as _C
    revealed = _opp_revealed_ids(obs)
    if len(revealed) < 2: return None, None
    rc = _C(revealed)
    scored = []
    for nm, ids in decks.items():
        dc = _C(ids)
        s = sum(min(rc[k], dc.get(k, 0)) * idf.get(str(k), 0.0) for k in rc)
        scored.append((s, nm, ids))
    scored.sort(key=lambda t: t[0], reverse=True)
    best_s, best_nm, best_deck = scored[0]
    second_s = scored[1][0] if len(scored) > 1 else 0.0
    # need real distinctive evidence AND clear separation from the runner-up
    if best_s >= float(os.environ.get('AZ_DETECT_MIN', '1.6')) and best_s >= second_s + 0.8:
        return best_nm, best_deck
    return None, None

def choose_main_az(obs, obs_dict):
    """POLICY net narrows to top-K candidates; VALUE net (via 1-ply search) picks the best."""
    if not _az_load() or obs.search_begin_input is None:
        return None
    # phase gate: the value net is sharp late (near the T14 buzzer) but noisy during setup, where
    # the rules already set up well. Skip AZ early (both sides still holding most prizes).
    _gate = int(os.environ.get('AZ_GATE', '4'))
    if min(len(me(obs).prize or []), len(opp(obs).prize or [])) > _gate:
        return None
    from cg.api import search_begin, search_step, search_end
    import numpy as np, bc_features as F
    me_i = obs.current.yourIndex; op = opp(obs)
    opts = (obs_dict.get("select") or {}).get("option") or []
    n = len(opts)
    if n <= 1: return None
    vocab, cf, adm = _AZ["vocab"], _AZ["cf"], _AZ["adm"]
    try:
        # 1) POLICY: score every option, keep the top-K candidates (+ the rule move)
        ofeat = np.array([F.option_features(obs_dict, opts[i], i, n, vocab, cf, adm) for i in range(n)],
                         dtype=np.float32)
        pol = _AZ["policy"].predict_proba(ofeat)[:, 1]
        K = int(os.environ.get('AZ_K', '5'))
        cand = list(np.argsort(-pol)[:K])
        rule = _validate(choose_main(obs), obs.select); rule_idx = rule[0] if rule else 0
        if rule_idx not in cand: cand.append(rule_idx)
        if len(cand) == 1: return [cand[0]]
        # 2) VALUE: search each candidate, score the resulting state. OPPONENT-AWARE determinization —
        # identify the archetype and fill the opp's hidden zones from a REAL decklist (minus revealed)
        # so a DEPTH-2 rollout simulates a real opp turn (without this the opp hand is energy fillers ->
        # sim opp can only attach+attack -> mirror-overfit). AZ_NDET averages several determinizations
        # to de-noise the deep look (a single guessed opp hand is high-variance).
        from collections import Counter as _C
        my_p = me(obs); need = my_p.deckCount + len(my_p.prize or [])
        opz_len = len(op.prize or []); oh_len = op.handCount or 0
        opp_nm, opp_deck = _detect_opp_deck(obs)
        oseen = _opp_revealed_ids(obs) if opp_deck is not None else None
        eff_depth = int(os.environ.get('AZ_DEPTH', '2')) if opp_deck is not None else 1  # deep only vs modeled opp
        ndet = int(os.environ.get('AZ_NDET', '1')) if opp_deck is not None else 1
        val_accum = np.zeros(len(cand), dtype=np.float64)
        for _det in range(ndet):
            pool = _unseen_pool(obs); random.shuffle(pool)
            pool += [5] * max(0, need - len(pool))
            yd = pool[:my_p.deckCount]; yp = pool[my_p.deckCount:need]
            if opp_deck is not None:
                oc = _C(opp_deck)
                for cid in oseen:
                    if oc.get(cid, 0) > 0: oc[cid] -= 1
                theirs = list(oc.elements()); random.shuffle(theirs)
                theirs += [5] * max(0, (op.deckCount + opz_len + oh_len) - len(theirs))
                ...
                od = [741] + [5] * max(0, op.deckCount - 1); opz = [5] * opz_len; oh = [5] * oh_len
            root = search_begin(obs, yd, yp, od, opz, oh, [741])
            feats, term = [], {}
            # DEPTH: turn-transitions to roll before scoring. 1 = end-of-OUR-turn (az0's 1-ply, blind
            # to the reply). 2 = also roll the opp's turn so the value net scores the board AFTER they
            # respond (did they KO us back?) — the tactical signal 1-ply misses. Bounded by AZ_MARGIN.
            for j, i in enumerate(cand):
                s = search_step(root.searchId, [int(i)]); o, d = s.observation, 0
                trans = 0; last_side = me_i
                while (o.select is not None and o.current.result == -1 and d < 80):
                    cur_side = o.current.yourIndex
                    if cur_side != last_side:
                        trans += 1; last_side = cur_side
                        if trans >= eff_depth:
                            break
                    s = search_step(s.searchId, _rule_dispatch(o)); o = s.observation; d += 1
                if o.current.result != -1:
                    term[j] = 1.0 if o.current.result == me_i else 0.0; feats.append([0.0] * F.N_STATE)
                else:
                    feats.append(F.state_features(o.current, me_i, vocab, cf, step=(getattr(o.current, 'turn', 0) or 0)))
            vv = _AZ["value"].predict_proba(np.array(feats, dtype=np.float32))[:, 1].astype(np.float64)
            for j, tv in term.items(): vv[j] = tv
            search_end()
            val_accum += vv
        vals = val_accum / max(1, ndet)
        # blend: value dominates; policy breaks near-ties
        polc = np.array([pol[i] for i in cand])
        score = vals + 0.05 * polc
        best = int(np.argmax(score))
        # confidence margin (env AZ_MARGIN, default 0 = always take AZ pick): only override the rule
        # move when the value net is CONFIDENTLY better -> bounds the downside to the rules.
        margin = float(os.environ.get('AZ_MARGIN', '0.06'))
        if margin > 0 and rule_idx in cand:
            rpos = cand.index(rule_idx)
            if float(score[best] - score[rpos]) < margin:
                return [int(rule_idx)]
...
def agent(obs_dict):
    ...
    try:
        ...
        if t == SelectType.MAIN:
            result = choose_main_az(obs, obs_dict)
            if result is None:
