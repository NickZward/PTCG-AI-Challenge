"""Alakazam pilot v21 (= v20 best-rules + MC value-search) — modern body (oracle, prize inference, guards)
...
CONFIG = {
 ...
 'COLLAPSE_BENCH': 2,
 'LILLIES_MAX_HAND': 5,
 'HILDA_MAX_HAND': 12,  # v20: REVERTED v15's tempo dial (10->12) — v13 (bigger hands) peaked 735 vs v15's 702
...
# ============ v18: MC-VALUE 1-PLY SEARCH over MAIN options ============
# For each MAIN option: apply it, let the RULE policy finish the turn, score the end-of-turn
# state with a VF (P(win)) trained on Monte-Carlo self-play values (analysis/gen_mc_data_ala.py
# -> train_vf_mc.py -> ala_vf_mc.joblib, renamed vf_model.joblib in this dir). Deviate from the
# rule choice only on a confident margin. Kaggle-safe: pure-rule fallback if joblib/sklearn/model
# missing. This is the project's ceiling lever — the one thing that can EXCEED the rules by
# picking WINNING moves via lookahead rather than cloning. (Adapted from grimmsnarl_v13.)
...
def choose_main_az(obs, obs_dict):
    ...
    try:
        ...
        # 2) VALUE: 1-ply search each candidate, score the end-of-turn state
        pool = _unseen_pool(obs); random.shuffle(pool); my_p = me(obs)
        need = my_p.deckCount + len(my_p.prize or []); pool += [5] * max(0, need - len(pool))
        yd = pool[:my_p.deckCount]; yp = pool[my_p.deckCount:need]
        od = [741] + [5] * max(0, op.deckCount - 1); opz = [5] * len(op.prize or [])
        oh = [5] * (op.handCount or 0)
        root = search_begin(obs, yd, yp, od, opz, oh, [741])
        feats, term = [], {}
        for j, i in enumerate(cand):
            s = search_step(root.searchId, [int(i)]); o, d = s.observation, 0
            while (o.select is not None and o.current.result == -1 and o.current.yourIndex == me_i and d < 40):
        ...
        vals = _AZ["value"].predict_proba(np.array(feats, dtype=np.float32))[:, 1]
        for j, tv in term.items(): vals[j] = tv
        search_end()
