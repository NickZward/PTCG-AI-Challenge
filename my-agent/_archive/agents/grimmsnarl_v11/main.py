...
def dynamic_fetch(obs):
    ...
    # 2. Attacker line: Impidimp -> (Candy | Morgrem) -> Grimmsnarl ex.
    if GRIMMSNARL not in play_ids and IMPIDIMP in play_ids:
        if GRIMMSNARL not in hand_ids:
            pri.append(GRIMMSNARL)
        if CANDY not in hand_ids and MORGREM not in hand_ids and MORGREM not in play_ids:
            pri += [CANDY, MORGREM]
...
def choose_cards(obs):
    ...
    if ctx == SelectContext.REMOVE_DAMAGE_COUNTER:
        # v11 (bono-learned, 90%): Munkidori moves the Froslass-inflicted counter
        # OFF ITSELF (benched Munkidori) — not off Grimmsnarl. Source = Munkidori.
        return pick_by_priority(obs, select, [MUNKIDORI] + cfg('ATTACH_TARGETS', []), count=select.maxCount)
