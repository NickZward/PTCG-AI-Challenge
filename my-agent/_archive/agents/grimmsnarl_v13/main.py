...
CONFIG = {
 ...
 'PROMOTE_PRIORITY': [GRIMMSNARL, MUNKIDORI, MORGREM, IMPIDIMP, FROSLASS, SNORUNT],
...
def choose_main(obs):
    ...
    if attach and not st.energyAttached:
        ...
        def score(o):
            ...
            # NOTE (2026-07-22): tried prioritizing Munkidori's 1st D energy here to power
            # the counter engine (Adrena-Brain needs a {D} on Munkidori; bono keeps it
            # energized 90% vs our 53%). REVERTED — ineffective: the bottleneck is Darkness
            # SUPPLY to hand (Punk Up pulls it from the deck for Grimmsnarl), not attach
            # priority, so reordering targets didn't move the energized rate (crustle 62=62,
            # mlucario 53->50). The real engine fix needs a supply/Froslass-uptime approach.
            if cid not in att_pri:
                arm = 4
            elif ready and en < atk_min:
                arm = 0 if is_active else 1
...
def choose_cards(obs):
    ...
    if ctx == SelectContext.REMOVE_DAMAGE_COUNTER:
        # bono-learned via OUTCOME-eval (analysis/outcome.py + learn_rmctr.py, n=711):
        # move counters off our MOST-DAMAGED body (protect whatever is closest to KO),
        # tie-break to the Grimmsnarl ex attacker. This is 76% agreement vs the old
        # "always Munkidori" rule's 45% (the old comment's "90%" was on the broken
        # raw-int agreement metric — re-audited and corrected).
        def rm_dmg_score(i):
            o = select.option[i]
            pi2 = o.playerIndex if o.playerIndex is not None else my_index
            c = card_at(obs, o.area, o.index, pi2)
            if c is None: return (-1, 0)
            dmg = (getattr(c, "maxHp", 0) or 0) - (getattr(c, "hp", 0) or 0)
            return (dmg, 1 if getattr(c, "id", None) == GRIMMSNARL else 0)
...
# For each MAIN option: apply it in the engine's search branch, let the RULE policy
# finish the turn, score the end-of-turn state with a VF (grimm_vf.joblib, P(win)),
# and deviate from the rule choice only on a confident margin. Kaggle-safe: pure-rule
# fallback if joblib/sklearn/model missing. VF trained on bono + our ladder Grimmsnarl
# games (test AUC .79, .90 mid-game). See analysis/train_vf.py.
_VF = {"model": None, "vocab": None, "deck": None, "ok": False}

def _vf_dirs():
    # Kaggle's exec runner leaves __file__ UNDEFINED, so fall back to cwd + the standard
    # kaggle agent dir. Without this the VF silently fails to load and search never fires.
    dirs = []
    try:
...
def _vf_load():
    ...
    for base in _vf_dirs():
        try:
            mp = os.path.join(base, "vf_model.joblib")
            vp = os.path.join(base, "bc_vocab.json")
            if not (os.path.exists(mp) and os.path.exists(vp)):
                continue
            if base not in _sys.path:
                _sys.path.insert(0, base)
            _VF["model"] = joblib.load(mp)
            _VF["vocab"] = _j.load(open(vp))
...
def choose_main_search(obs):
    ...
    if not _VF["ok"] or obs.search_begin_input is None:
        return None
    # Phase gate: VF is sharpest at min-prize<=3 (mid/late, AUC .74-.90); skip the opening.
    _mp = len(me(obs).prize or []); _op = len(opp(obs).prize or [])
    if min(_mp, _op) > int(os.environ.get('VF_GATE', '3')):
        return None
    from cg.api import search_begin, search_step, search_end
    ...
    try:
        ...
        n = len(obs.select.option); feats, terminal = [], {}
        for i in range(n):
            s = search_step(root.searchId, [i]); o, depth = s.observation, 0
            # let the rule policy finish the ENTIRE turn, then score where it lands
            while (o.select is not None and o.current.result == -1
                   and o.current.yourIndex == me_i and depth < 40):
                s = search_step(s.searchId, _rule_dispatch(o)); o = s.observation; depth += 1
            if o.current.result != -1:
                ...
                feats.append(F.state_features(o.current, me_i, vocab, cf,
                                              step=(getattr(o.current, 'turn', 0) or 0)))
        ...
        margin = float(os.environ.get('VF_MARGIN', '0.05'))
...
def agent(obs_dict):
    ...
    try:
        t = select.type
        if t == SelectType.MAIN: result = choose_main_search(obs) or choose_main(obs)
        elif t in (SelectType.CARD, SelectType.CARD_OR_ATTACHED_CARD, SelectType.ATTACHED_CARD):
