...
def choose_cards(obs):
    ...
    if eff is not None and getattr(eff, 'id', None) == GRIMMSNARL:
        ...
        if ctx == SelectContext.ATTACH_TO and select.option and \
            ...
            # v15 JULY-META CALIBRATION: ALL FOUR July top pilots (S4nkurero 60%,
            # Yushin 57%, tonakaiiii 56%, kazuki0123 53%; 1000 games profiled) run Punk Up
            # to stack-mode TWO, not three — the July Grimm-saturated meta is an engine/
            # chip game (<1 attack/game at the top table); the 3rd energy arrives via a
            # manual attach only when a swing is actually taken. Take deficit-to-2; the
            # arm ladder still tops the active to 3 for real attacks.
            def deficit2(pkm):
                return max(0, 2 - energy_count(pkm))
            need = sum(sorted((deficit2(p) for _, _, p in board), reverse=True)[:1])
