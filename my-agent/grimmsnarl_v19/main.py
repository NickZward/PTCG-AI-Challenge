...
def opp_max_damage(obs):
    """Printed max attack damage the opponent's ACTIVE can deal (base; ignores their energy/
    scaling -> a CONSERVATIVE under-estimate, so the exposure gate only fires when clearly doomed;
    catches the wounded-ex majority, under-flags fresh-ex-into-scaling which is fine = fewer fires)."""
    oa = opp(obs).active
    if not (oa and oa[0]):
        return 0
    ci = card_info(oa[0].id)
    if ci is None or not getattr(ci, "attacks", None):
        return 0
    return max((attack_damage(aid) for aid in ci.attacks), default=0)
...
def choose_main(obs):
    ...
    # 3.7 PRIZE-EXPOSURE RETREAT (v18, fresh-replay autopsy 2026-07-25): the #1 close-loss cluster
    # (10/19 winnable losses) is feeding a 2-prize GrimmEX into a KO it can't survive while a
    # 1-prize chump sits benched -> opp banks 2 prizes not 1, and we drop the T14 cap by ~1.
    # Reaching HERE means we could NOT take a KO/prize this turn (3.5/3.6 already returned if we
    # could), so the ex is only chip-attacking. If the active is a 2-prize body that will be OHKO'd
    # next turn, a 1-prize body is benched, and we're not deep-behind, retreat so the incoming KO
    # costs 1 prize not 2. Strict do-no-harm: no offense skipped (KO logic ran first), OFF vs walls
    # that can't OHKO us (doomed=False -> Crustle/engine-underfire games untouched), never fires
    # without a chump, only in the cap-tiebreak zone (ahead/even/down-1). Also fixes the mirror
    # double-KO case (a doomed ex retreats behind a body instead of dying for 2 while idle).
    if (os.environ.get('EXPO_FIX', '1') == '1' and retreat_idx is not None and not st.retreated
            and my.active and my.active[0] is not None):
        _act = my.active[0]
        _two = cfg('TWO_PRIZE', {GRIMMSNARL})
        if _act.id in _two:
            # SKIP THE MIRROR: vs another 2-prize GrimmEX the KO trades 2-for-2 and we punish back,
            # so retreating just loses tempo (over-fires -> passive; A/B craters the mirror). Only
            # protect vs decks where the KO on us is a CLEAN profit (heal-walls / scaling / low-prize
            # attackers we can't trade evenly into). Detect the mirror by the opp's revealed line.
            _op = opp(obs)
            _mirror = any(p is not None and p.id in (GRIMMSNARL, MORGREM, IMPIDIMP, MUNKIDORI)
                          for p in (list(_op.active or []) + list(_op.bench or [])))
            # doomed = the opp can OHKO our active. Take the MAX of the printed attack damage and the
            # OBSERVED OHKO (v19): if a scaling attacker has already OHKO'd one of our GrimmEX this
            # game, even a full-HP ex is doomed -> stop feeding it, sandbag behind a 1-prize chump.
            _obs = _TRK.get('opp_max_dmg', 0) if os.environ.get('OBS_DMG', '1') == '1' else 0
            _doomed = max(opp_max_damage(obs), _obs) >= (_act.hp or 0)
            _chump = any(b is not None and b.id not in _two for b in (my.bench or []))
            _myp = 6 - len(my.prize or []); _opz = 6 - len(_op.prize or [])
            if _doomed and _chump and not _mirror and _myp + 1 >= _opz:
                _INSTR['expo_retreat'] = _INSTR.get('expo_retreat', 0) + 1
                return [retreat_idx]
...
def _track(obs):
    ...
    # OBSERVED-OHKO tracking (v19): the opponent's PRINTED attack damage under-flags SCALING
    # attackers (Alakazam 180->340), so a full/near-full GrimmEX that a scaled attacker will OHKO
    # isn't flagged as doomed and gets fed for a 2-for-1. Here we record proof-of-OHKO: when one of
    # our 2-prize GrimmEX vanishes from the board, the opp KO'd it (GrimmEX is terminal -> a gone
    # serial is a real KO, never an evolution artifact), so they can deal >= its last-seen HP. That
    # observed number then broadens the 3.7 exposure retreat below. last-seen HP is a LOWER bound on
    # the hit (conservative -> won't over-fire).
    my = st.players[my_i]
    _live = {}
    for c in [a for a in (my.active or []) if a] + [b for b in (my.bench or []) if b]:
        if getattr(c, "id", None) == GRIMMSNARL:
            ser = getattr(c, "serial", None)
            if ser is not None:
                _live[ser] = getattr(c, "hp", 0) or 0
    for ser, hp in _TRK["ex_hp"].items():
        if ser not in _live:
            _TRK["opp_max_dmg"] = max(_TRK["opp_max_dmg"], hp)
    _TRK["ex_hp"] = _live
...
def choose_cards(obs):
    ...
    if ctx in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
        ...
        def score(i):
            ...
            # NOTE (2026-07-25): tried a "don't chump a dry 2-prize GrimmEX" sacrifice-discipline tweak
            # from the M-Starmie autopsy — REVERTED. 2-run A/B: no measurable Starmie benefit vs the
            # gauntlet proxy (fix 56.7/60 vs base 55/60) AND it regressed the mirror (36.7/46, both <50,
            # because a tanky dry GrimmEX SURVIVES and threatens in the mirror where 2-prize gifts hurt
            # less). Matchup-dependent -> would need Starmie-detection gating + ladder validation (the
            # gauntlet Starmie can't replicate the real 50-snipe). See [[ptcg-starmie-autopsy]].
