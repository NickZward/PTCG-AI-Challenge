#!/usr/bin/env python3
"""Replay @kdcyberdude's own 135 games and record, at every decision HE faced, what ONE of our
nets would pick. Runs in its OWN PROCESS per agent -> zero module-sharing risk (the
mcts_agent/np_common cache-slot hazard that field_gauntlet._purge_private exists to fix).

Two equivalence notions are recorded per decision:
  ENC group  = byte-identical decoder encoding (the net is structurally unable to prefer one).
  PLAY sig   = the same GAME ACTION. Strictly coarser than ENC because FEAT-v2 puts the source
               card's HAND-SLOT index in the candidate encoding: attaching the 1st vs the 4th
               identical Basic {D} Energy to the same Munkidori are four different encodings and
               four different logits, but ONE play. Hand-verified on 91149902 step 69.

Usage: kd_probe.py <agent_dir> <out.jsonl>
"""
...
OUT = sys.argv[2]
REPLAYS = "/Users/nickzwart/Desktop/kdcyberdude"

sys.path.insert(0, AGENT)              # this agent's OWN np_common / cg, front of path
import numpy as np
...
def opt_sig(oc, i):
    """Identity of the GAME ACTION this option performs. Interchangeable copies of the same card
    in a hidden/unordered zone (hand, deck, discard, prize) collapse to one signature; distinct
    in-play Pokemon never do."""
...
def opt_label(oc, i):
    ...
    try:
        ...
        if t == OptionType.ATTACH:
            return "ATTACH:%s>%s@%s%s" % (cname(NN.get_card(oc, o.area, o.index, yi)),
                                          cname(NN.get_card(oc, o.inPlayArea, o.inPlayIndex, yi)),
                                          AT.get(int(o.inPlayArea), o.inPlayArea), o.inPlayIndex)
        if t == OptionType.EVOLVE:
            return "EVOLVE:%s>%s@%s%s" % (cname(NN.get_card(oc, o.area, o.index, yi)),
                                          cname(NN.get_card(oc, o.inPlayArea, o.inPlayIndex, yi)),
                                          AT.get(int(o.inPlayArea), o.inPlayArea), o.inPlayIndex)
        if t == OptionType.ABILITY:
            return "ABILITY:%s@%s%s" % (cname(NN.get_card(oc, o.area, o.index, yi)),
                                        AT.get(int(o.area), o.area), o.index)
...
def main():
    ...
    for f in files:
        try:
            ...
            continue
        tn = d["info"]["TeamNames"]
        seats = [i for i, n in enumerate(tn) if "kdcyberdude" in (n or "").lower()]
        if not seats:
            continue
        pi = seats[0]
        S = d["steps"]
        for t in range(len(S) - 1):
            ...
            try:
                ...
                if len(actions) <= 1:
                    continue                      # forced move, no decision
            ...
            # distinct PLAYS, ranked by their best candidate
            sig_ids, sid = [], {}
            for s in sigs:
                sid.setdefault(s, len(sid)); sig_ids.append(sid[s])
            best = {}
            for k in order:                       # order is descending policy, first wins
            ...
            top = order[0]
            rec = {
                "f": os.path.basename(f), "t": t, "seat": pi, "turn": oc.current.turn,
                "stype": int(sel.get("type")), "sctx": int(sel.get("context")),
                "nopt": len(opts), "mn": sel.get("minCount"), "mx": sel.get("maxCount"),
                "ncand": len(actions), "ngroups": ngroups, "nplays": len(sid),
                "hidx": hidx, "top": top,
