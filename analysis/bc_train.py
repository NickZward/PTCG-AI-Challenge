...
import sys as _sys
PLAYER = _sys.argv[1] if len(_sys.argv) > 1 else 'bono'
FOLDER = _sys.argv[2] if len(_sys.argv) > 2 else f"{ROOT}/Bono"
OUTM   = _sys.argv[3] if len(_sys.argv) > 3 else f"{ROOT}/model/bono_model.joblib"
MAXG   = int(_sys.argv[4]) if len(_sys.argv) > 4 else 100000
def bono_idx(d):
    for i, n in enumerate(d['info']['TeamNames']):
        if PLAYER.lower() in n.lower(): return i
    return 0

# ---- extract bono decisions ----
X, y, group, ctxs, game = [], [], [], [], []   # group = per-DECISION id; game = per-GAME id
files = sorted(glob.glob(f"{FOLDER}/*.json"))[:MAXG]
gid = 0
dec = 0
for f in files:
    try:
...
_ITER=int(os.environ.get('HISTGB_ITER','500'))
clf = HistGradientBoostingClassifier(max_iter=_ITER, learning_rate=0.08, max_depth=None,
                                     min_samples_leaf=40, l2_regularization=1.0,
                                     early_stopping=(_ITER>=400), validation_fraction=0.1, random_state=0)
clf.fit(X[tr], y[tr])
