...
sys.path.insert(0, os.path.join(ROOT, "my-agent/dragapult_v4"))
import bc_features as F

VOCABJ = json.load(open(os.path.join(ROOT, "my-agent/dragapult_v4/bc_vocab.json")))
...
def collect(files, who, grimm_only):
    ...
    for p in files:
        ...
        for step in d['steps']:
            ...
            seen_turn.add(turn)
            try:
                feat = F.state_features(cur, me, VOCAB, CF, step=turn)
            except Exception:
                continue
...
def main():
    ...
    clf_full.fit(X, y)
    import joblib
    out = os.path.join(ROOT, "model/grimm_vf.joblib")
