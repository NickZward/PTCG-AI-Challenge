...
def _load_agent(sub, nm):
    spec = _ilu.spec_from_file_location(nm, os.path.join(ROOT, sub, "main.py"))
    m = _ilu.module_from_spec(spec); sys.modules[nm] = m; spec.loader.exec_module(m)
    return m.agent

AGENT_FN = _load_agent(AGENT, "genv_agent")
DECK = [int(x) for x in open(os.path.join(AGENT, "deck.csv")).read().split()]
NN.DEFAULT_FEAT[0] = 2

if OPP == "mirror":
    OPP_FN, OPP_DECK = AGENT_FN, DECK
else:
    OPP_FN = _load_agent(OPP, "genv_opp")
...
def play_one(mirror, our_seat):
    ...
    try:
        while steps < 4000:
            ...
            fn = AGENT_FN if ours else OPP_FN
                ...
                try:
                    ...
                    if oc.select is not None:
                        ...
                        if len(acts) >= 2:
                            ...
                            svd = NN.get_decoder_input(oc, acts, 2)
...
def main():
    ...
    data["feat"] = np.asarray([2])
    data["decoder_vocab"] = np.asarray([NN.decoder_size_v2])
